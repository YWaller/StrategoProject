# -*- coding: utf-8 -*-
"""
Created on Fri Dec 22 18:41:13 2017
...
class mfile(pygame.sprite.Sprite):
    ...
    def show_moves(self):
        if self.level == 11:
            ...
            stepper = 50
            negstepper = -50
            cellintervals=range(lim[1],150,negstepper)
            staticxy=lim[0]
            up = [[a]-[staticxy] for a in cellintervals]
            cellintervals=range(lim[1],650,stepper)
            down = [[a]+[staticxy] for a in cellintervals]
            cellintervals=range(lim[1],50,negstepper)
            staticxy=lim[1]
